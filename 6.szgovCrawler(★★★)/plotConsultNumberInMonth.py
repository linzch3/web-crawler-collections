...
for filename in first31files.filename:
    ...
    #截取日期的月份信息
    date=[date[5:7] for date in df.询问日期]
    #计算每月的咨询数目并根据重复数量排序
    ...
    for date in newdf.index:
        newdate.append(datetime.datetime.strptime(date, '%M'))
    ...
    ax.set_xlabel('月份',fontproperties='SimHei',fontsize=15)
...
#    plt.show()
#    break
    filepath='问题咨询数量的月份变化图/'+filename.split('.')[0]+'_问题咨询数量的月份变化图.jpg'
